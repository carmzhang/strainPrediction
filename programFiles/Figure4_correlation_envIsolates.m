...
% 153. Figure 4, Supplementary Figure 2.4
%
...
data = Data_copy(:, 2:end);
label_temp = Data_copy(:, 1);%bdr#
s = size(data);
%1D median filter (window = 3)
for i = 1:1:s(1)
    data(i, :) = medfilt1(data(i, :));
end
...
for i = 1:length(label_temp)
    idx = find(match_fin(:, 1) == label_temp(i));
    if(~isempty(idx))
        label = [label; match_fin(idx, 2)];%FS#
        data_matched = [data_matched; data(i, :)];
    end
end
...
%fs # of sequenced isolates
seq = [];
for i = 1:size(match_id{:, 15}, 1)
    seq = [seq, str2double(match_id{i, 15}{1}(3:end))];
end
seq_fin = seq(~isnan(seq));
...
data_m = reshape(mean(reshape(data,12,[])),[],99);%mean of 12 replicates
...
for k = 1:numel(sheet_name)
    ...
    isolates = unique(intersect(isolates, label));
...
order = 4;%change this line to change order
