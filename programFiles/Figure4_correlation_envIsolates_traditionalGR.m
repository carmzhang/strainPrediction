...
% 145. This script will produce plots for Figure 4 and Supplementary 
% Figure 2.4 using growth rate as the features.
%
...
label_temp = Data_copy(:, 1);
s = size(data);
%1D moving average filter (window = 5)
for i = 1:1:s(1)
    data(i, :) = conv2(data(i, :), ones(1, 5)/5, 'same');
end
...
s = size(data_matched);
data = data_matched;
data = data - min(min(data))+0.0000001;

%convert raw data to derivative
time = 10/60;
data = gradient(log(data), time);
s = size(data);
data = [label, data];
