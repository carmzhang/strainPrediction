...
for i=1:K
    ...
    if(strcmp(kernel, 'rbf') == 1)
        SVMStruct{i} = svmtrain(train_feature,train_label, 'kernel_function', char(kernel),'boxconstraint', cost, 'rbf_sigma', sigma, 'kktviolationlevel', kkt, 'options', statset('MaxIter', 10000000));
    else
