...
%                * features are derivative of growth curves
...
for i = 1:1:width(ord)
    o_all = [o_all; o_seq{i}];
    sum_o = sum_o + length(o_seq{i});
    len = [len, length(o_seq{i})];
end
...
% match_id = readtable('Environmental isolates_information.xlsx', 'TreatAsEmpty', 'N/A');
% match_fin = [match_id{:, 2}, match_id{:, 4}];%column 1: bdr#, 2: FS#
...
for i = 1:1:s(1)
    data_seq(i, :) = medfilt1(data_seq(i, :));
end
